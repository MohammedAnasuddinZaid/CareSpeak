...
import { Hand, Mic, RotateCcw, Camera, StopCircle, Pause, ThumbsUp, ThumbsDown, HelpCircle, Droplets, Sparkles, Activity, Fingerprint, Volume2 } from "lucide-react";
import { useHandGesture } from "@/hooks/useHandGesture";
import { HAND_GESTURE_MAP } from "@/types";
import { voiceAlert } from "@/lib/tts";

const GESTURE_GUIDE = [
  { label: "YES", desc: "Thumbs Up", icon: ThumbsUp, gradient: "from-emerald-500 to-teal-600" },
  { label: "NO", desc: "Thumbs Down", icon: ThumbsDown, gradient: "from-red-500 to-rose-600" },
  { label: "HELP", desc: "Index & Pinky Extended", icon: HelpCircle, gradient: "from-amber-500 to-orange-600" },
  { label: "WATER", desc: "Both Hands Open", icon: Droplets, gradient: "from-cyan-500 to-blue-600" },
  { label: "PAUSE", desc: "Open palm wide 5s to toggle", icon: Pause, gradient: "from-slate-500 to-slate-600" },
];
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="relative overflow-hidden rounded-3xl aspect-[4/3] dashboard-card">
              {loading && (
                <div className="absolute inset-0 flex items-center justify-center bg-slate-950/90 z-20">
                  <div className="text-center">
                    ...
                    <p className="text-slate-300 text-sm font-medium">Loading hand tracking model...</p>
              ...
              {cameraOn && (
                <div className="absolute top-4 left-4 px-3 py-1.5 rounded-xl bg-black/50 backdrop-blur-sm text-white/80 text-xs font-medium flex items-center gap-2">
                  <span className={`w-2 h-2 rounded-full ${isPaused ? "bg-amber-500" : numHands > 0 ? "bg-emerald-500" : "bg-slate-500"}`} />
            ...
            {cameraOn && (
              <div className="dashboard-card p-4 flex items-center gap-3">
                <span className={`flex items-center gap-1.5 text-sm ${isPaused ? "text-amber-400" : numHands > 0 ? "text-emerald-400" : "text-slate-500"}`}>
                  <span className={`w-2 h-2 rounded-full ${isPaused ? "bg-amber-500" : numHands > 0 ? "bg-emerald-500 animate-pulse" : "bg-slate-600"}`} />
            ...
            <AnimatePresence mode="wait">
              {isPaused ? (
                ...
                >
                  ...
                  <div className="text-xs text-slate-500">Show open palm wide to resume</div>
                </motion.div>
              ) : gesture && confidence > 0.5 ? (
                ...
                >
                  ...
                  <p className="text-slate-400 font-medium">Waiting for gesture...</p>
                  <p className="text-slate-600 text-xs mt-1">Make a gesture to see the result</p>
                </motion.div>
              )}
            </AnimatePresence>
